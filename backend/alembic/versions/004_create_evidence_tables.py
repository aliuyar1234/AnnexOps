...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('type', postgresql.ENUM('upload', 'url', 'git', 'ticket', 'note', name='evidence_type', create_type=False), nullable=False),
        sa.Column('title', sa.String(255), nullable=False),
        sa.Column('description', sa.Text, nullable=True),
        sa.Column('tags', postgresql.ARRAY(sa.Text), nullable=False, server_default='{}'),
        sa.Column('classification', postgresql.ENUM('public', 'internal', 'confidential', name='classification', create_type=False), nullable=False, server_default='internal'),
    ...
    op.create_table(
        'evidence_mappings',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('uuid_generate_v4()')),
        sa.Column('evidence_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('evidence_items.id', ondelete='CASCADE'), nullable=False),
        sa.Column('version_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('system_versions.id', ondelete='CASCADE'), nullable=False),
        sa.Column('target_type', postgresql.ENUM('section', 'field', 'requirement', name='mapping_target_type', create_type=False), nullable=False),
        sa.Column('target_key', sa.String(100), nullable=False),
        sa.Column('strength', postgresql.ENUM('weak', 'medium', 'strong', name='mapping_strength', create_type=False), nullable=True),
